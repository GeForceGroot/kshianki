...
const SuperKingSizeBedSheets = ({ products }) => {
    return (
        <>
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap -m-4 justify-center">
            {Object.keys(products).length === 0 && <p>Sorry All The Super King SizeBed Sheets are Currently Out Of Stock. New Stock Coming Soon!</p>}
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: 'superKingSizeBedSheets'})

  let superKingSizeBedSheets = {}

  for (let item of products) {

    if (item.title in superKingSizeBedSheets) {

      if (!superKingSizeBedSheets[item.title].color.includes(item.color) && item.availableQty > 0) {
        superKingSizeBedSheets[item.title].color.push(item.color)
      }

    }
    else {
      superKingSizeBedSheets[item.title] = JSON.parse(JSON.stringify(item))
      if (item.availableQty > 0) {
        superKingSizeBedSheets[item.title].color = [item.color]
      }
    }
  ...
  return {
    ...
    props: { products : JSON.parse(JSON.stringify(superKingSizeBedSheets)) },

  }
}

export default SuperKingSizeBedSheets
