...
const KingSizeElaticFittedBedSheet = ({ products }) => {
    return (
        <>
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap -m-4 justify-center">
            {Object.keys(products).length === 0 && <p>Sorry All The King Size Elatic Fitted Bed Sheet are Currently Out Of Stock. New Stock Coming Soon!</p>}
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: 'kingSizeElaticFittedBedSheet'})

  let kingSizeElaticFittedBedSheet = {}

  for (let item of products) {

    if (item.title in kingSizeElaticFittedBedSheet) {

      if (!kingSizeElaticFittedBedSheet[item.title].color.includes(item.color) && item.availableQty > 0) {
        kingSizeElaticFittedBedSheet[item.title].color.push(item.color)
      }

    }
    else {
      kingSizeElaticFittedBedSheet[item.title] = JSON.parse(JSON.stringify(item))
      if (item.availableQty > 0) {
        kingSizeElaticFittedBedSheet[item.title].color = [item.color]
      }
    }
  ...
  return {
    ...
    props: { products : JSON.parse(JSON.stringify(kingSizeElaticFittedBedSheet)) },

  }
}

export default KingSizeElaticFittedBedSheet
